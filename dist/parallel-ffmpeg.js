...
import { buildAssembleArgs, buildAudioArgs, buildAudioProbeArgs, buildConcatManifest, buildSegmentTranscodeArgs, buildSplitArgs, segmentName, validateOptions, } from "./internal/commands.js";
...
export class ParallelFFmpeg {
    ...
    async transcode(input, options) {
        ...
        const emitProgress = (stage, rawStageRatio, completedSegments, totalSegments, activeSegment) => {
            ...
            if (activeSegment !== undefined)
                event.activeSegment = activeSegment;
            safeNotify(options.onProgress, event, "onProgress");
        };
        const planner = this.#newEngine("planner");
        const plannerContext = { scope: "planner", recentLogs: [] };
        let plannerLogHandler;
        let segments = [];
        let audio;
        let plannerProgressStage;
        const plannerProgressHandler = ({ progress }) => {
        ...
        try {
            ...
            if (audioStrategy === "single-pass") {
                ...
                audio = await this.#extractAudio(planner, inputName, audioName, probeName, audioArgs, options, plannerContext);
            ...
            segments = new Array(names.length);
            for (let index = 0; index < names.length; index += 1) {
                this.#throwIfAborted(options.signal);
                const name = names[index];
                if (!name)
                    throw new Error("Internal segment indexing error.");
                segments[index] = requireBinary(await planner.readFile(name, "binary", messageOptions(options.signal)), `Reading ${name}`);
        ...
        const workerContexts = workers.map((_, worker) => ({
            scope: "worker",
            worker,
            recentLogs: [],
        }));
        const workerLogHandlers = new Array(workerCount).fill(undefined);
        const workerProgress = new Array(workerCount).fill(0);
        const workerActive = new Array(workerCount).fill(false);
        const workerSegments = new Array(workerCount).fill(undefined);
        let completed = 0;
        const emitTranscodeProgress = (workerIndex) => {
            const activeProgress = workerProgress.reduce((sum, value, index) => sum + (workerActive[index] ? value : 0), 0);
            const ratio = (completed + activeProgress) / segmentCount;
            emitProgress("transcoding", ratio, completed, segmentCount, workerIndex === undefined ? undefined : workerSegments[workerIndex]);
        };
        this.#throwIfAborted(options.signal);
        ...
        try {
            await Promise.all(workers.map(async (worker, workerIndex) => {
                ...
                if (!context)
                    throw new Error(`Missing log context for worker ${workerIndex}.`);
        ...
        try {
            encodedSegments = await runIndexedPool(workers, segmentCount, async (worker, workerIndex, segmentIndex) => {
                ...
                const context = workerContexts[workerIndex];
                if (!bytes)
                    throw new Error(`Missing source segment ${segmentIndex}.`);
                if (!context)
                    throw new Error(`Missing log context for worker ${workerIndex}.`);
                ...
                try {
                    ...
                    const output = requireBinary(await worker.readFile(encodedName, "binary", messageOptions(options.signal)), `Reading encoded segment ${segmentIndex}`);
        ...
        try {
            ...
            for (let index = 0; index < encodedSegments.length; index += 1) {
                ...
                if (!bytes)
                    throw new Error(`Missing encoded segment ${index}.`);
            ...
            await assembler.writeFile(manifestName, buildConcatManifest(outputSegmentNames), messageOptions(options.signal));
            if (audio)
            ...
            const data = requireBinary(await assembler.readFile(outputName, "binary", messageOptions(options.signal)), "Reading final output");
    ...
    async #extractAudio(engine, inputName, audioName, probeName, audioArgs, options, context) {
        if (!engine.ffprobe) {
            throw new Error('audioStrategy="single-pass" requires an FFmpeg engine with ffprobe support.');
        }
        {
            const probeCode = await engine.ffprobe(buildAudioProbeArgs({ inputName, outputName: probeName }), options.execTimeoutMs ?? -1, messageOptions(options.signal));
        ...
        const code = await this.#exec(engine, buildAudioArgs({ inputName, outputName: audioName, audioArgs }), options);
        this.#assertExitCode(code, "Single-pass audio extraction", context);
        try {
            const audio = requireBinary(await engine.readFile(audioName, "binary", messageOptions(options.signal)), "Reading single-pass audio");
    ...
    async #loadEngine(engine, config, options, context) {
        const logHandler = ({ message }) => {
            context.recentLogs.push(message);
            if (context.recentLogs.length > MAX_RECENT_LOGS)
                context.recentLogs.shift();
    ...
    async #exec(engine, args, options) {
        return engine.exec(args, options.execTimeoutMs ?? -1, messageOptions(options.signal));
    }
    #assertExitCode(code, operation, context) {
        if (code === 0)
            return;
    ...
    #emitLog(options, context, message) {
        const event = context.worker === undefined
...
function disposeEngine(engine, logHandler) {
    if (logHandler)
        safeRemoveLogListener(engine, logHandler);
    safeTerminate(engine);
}
function safeRemoveLogListener(engine, handler) {
    try {
...
function safeRemoveProgressListener(engine, handler) {
    try {
...
async function safeDelete(engine, path) {
    try {
...
function messageOptions(signal) {
    return signal ? { signal } : undefined;
}
function createInternalPrefix(inputName, outputName) {
    let prefix;
    do {
        runSequence += 1;
...
function safeNotify(callback, event, label) {
    if (!callback)
        return;
    try {
        callback(event);
...
//# sourceMappingURL=parallel-ffmpeg.js.map