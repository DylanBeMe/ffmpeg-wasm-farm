...
import { buildAssembleArgs, buildAudioArgs, buildAudioProbeArgs, buildConcatManifest, parseMediaTimingProbe, buildSegmentTranscodeArgs, buildSplitArgs, segmentName, validateOptions, } from "./internal/commands.js";
...
export class ParallelFFmpeg {
    ...
    static recommendedWorkerCount(inputBytes = 0) {
        ...
        try {
            ...
            try {
                ...
                if (audioStrategy === "single-pass") {
                    ...
                    const extractedAudio = await this.#extractAudio(planner, inputName, audioName, probeName, audioArgs, options, plannerContext);
                    if (extractedAudio) {
                ...
                for (let index = 0; index < names.length; index += 1) {
                    ...
                    const bytes = requireBinary(await planner.readFile(name, "binary", messageOptions(options.signal)), `Reading ${name}`);
    ...
    async #extractAudio(engine, inputName, audioName, probeName, audioArgs, options, context) {
        ...
        }
        {
            ...
            const code = await this.#exec(engine, buildAudioArgs({
                inputName,
...
function intermediateSegmentKey(kind, index) {
