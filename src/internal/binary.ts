...
export async function toUint8Array(input: BinaryInput, preserve: boolean): Promise<Uint8Array> {
  if (input instanceof Uint8Array) {
    ...
    if (isSharedArrayBuffer(input.buffer)) return input.slice();
...
export function inferInputName(input: BinaryInput, explicit?: string): string {
  if (explicit !== undefined) return sanitizeFilename(explicit);
  ...
  throw new Error(
    "inputName is required when the input is not a named File and its format cannot be inferred.",
  );
}

export function requireBinary(data: Uint8Array | string, context: string): Uint8Array {
...
export function sanitizeFilename(name: string): string {
  if (typeof name !== "string") throw new TypeError("Filename must be a string.");
...
function sniffExtension(bytes: Uint8Array): string | undefined {
  if (matches(bytes, 0, [0x1a, 0x45, 0xdf, 0xa3])) return "mkv";
  ...
  if (riff && ascii(bytes, 8, 4) === "WAVE") return "wav";
  if (riff && ascii(bytes, 8, 4) === "AVI ") return "avi";

  if (ascii(bytes, 4, 4) === "ftyp") return "mp4";
  if (matches(bytes, 0, [0x00, 0x00, 0x01, 0xba])) return "mpeg";
...
function isTransportStream(bytes: Uint8Array): boolean {
  if (bytes.length < 377) return false;
